(function(){
    ...
    clock.prototype.drawCle = function(x, y) { // 初始化解锁密码面板 小圆圈
    ...
    clock.prototype.drawPoint = function(style) { // 初始化圆心
    ...
    clock.prototype.drawStatusPoint = function(type) { // 初始化状态线条
        for (var i = 0 ; i < this.lastPoint.length ; i++) {
    ...
    clock.prototype.drawLine = function(style, po, lastPoint) {//style:颜色 解锁轨迹
        this.ctx.beginPath();
    ...
    clock.prototype.createCircle = function() {// 创建解锁点的坐标，根据canvas的大小来平均分配半径
    ...
    clock.prototype.update = function(po) {// 核心变换方法在touchmove时候调用
        this.ctx.clearRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.height);
...
// }
    ...
    clock.prototype.checkPass = function(psw1, psw2) {// 检测密码
        var p1 = '',
        p2 = '';
    ...
    clock.prototype.storePass = function(psw) {// touchend结束之后对密码和状态的处理
    ...
    clock.prototype.updatePassword = function(){
    ...
    clock.prototype.initDom = function(){
        ...
        var str = '<h4 id="title" class="title" style="color:#87888a">请绘制您的图形密码</h4>'+
                  ...
                  '<a style="position:absolute;left:5px;bottom:5px;color:#fff;font-size:10px;">紧急呼叫</a>'+
                  '<a style="position:absolute;right:5px;bottom:5px;color:#fff;font-size:10px;">返回</a>';
    ...
    clock.prototype.bindEvent = function() {
        var self = this;
